...
use crate::cipher::{
    RPG_Client_EquipmentItemData, RPG_Client_InventoryModule, RPG_Client_RelicItemData,
    RPG_Client_TextmapStatic, RPG_GameCore_AvatarPropertyExcelTable, RPG_GameCore_FixPoint,
...
use crate::models::{
    LightCone, Relic, RelicMainStat, RelicRolls, RelicSubstat, ReliquaryLightCone, ReliquaryRelic,
};
use crate::relic_utils::{
    calc_initial_rolls, get_light_cones, get_relics, pick_low_mid_high, write_light_cones_to_json,
    write_relics_to_json,
};
...
fn update_relics(this: RPG_Client_InventoryModule, _list: List, flag: bool) {
    unsafe {
        ...
        _UpdateRelics_Detour.call(this, _list, flag);
...
fn update_equipments(this: RPG_Client_InventoryModule, _list: List, flag: bool) {
    unsafe {
        ...
        _UpdateEquipments_Detour.call(this, _list, flag);
...
fn sync_equipment(this: RPG_Client_EquipmentItemData, packet: *const c_void) {
    unsafe {
        sync_equipment_Detour.call(this, packet);
...
fn process_equipment_data(this: RPG_Client_EquipmentItemData) -> Result<ReliquaryLightCone> {
    unsafe {
        ...
        let name =
            RPG_Client_TextmapStatic::get_text(&*equipment_row.EquipmentName()?, std::ptr::null())?;
        ...
        get_light_cones()
            .write()
            .insert(uid.to_string(), light_cone);
...
fn sync_relic(this: RPG_Client_RelicItemData, packet: *const c_void) {
    unsafe {
        sync_relic_Detour.call(this, packet);
    ...
    unsafe {
        hook_fn!(
            ...
            RPG_Client_RelicItemData::get_class_static()?
                .methods()
                .iter()
                .find(|method| { method.name() == "Sync" && method.args_cnt() == 1 })
        ...
        hook_fn!(
            ...
            RPG_Client_InventoryModule::get_class_static()?
                .methods()
                .iter()
                .find(|method| { method.name() == "_UpdateRelics" && method.args_cnt() == 2 })
        ...
        hook_fn!(
            ...
            RPG_Client_InventoryModule::get_class_static()?
                .methods()
                .iter()
                .find(|method| { method.name() == "_UpdateEquipments" && method.args_cnt() == 2 })
